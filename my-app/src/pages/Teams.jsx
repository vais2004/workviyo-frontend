    ...
    <div className="container-fluid">
      <div className="row">
        ...
        <div className="col-12 col-md-9 col-lg-10 p-4">
          ...
          <section className="pb-3 px-2">
            <div className="row">
              {status === "Loading" && <p className="text-center p-3 bg-primary-subtle">Loading...</p>}
              {error && <p className="text-center p-3 bg-warning-subtle">{error}</p>}

              {Array.isArray(teams) && teams.length > 0 && teams.map((team) => (
                <div className="col-md-4 py-2" key={team._id}>
                  <Link style={{ textDecoration: "none" }} to={`/teamDetail/${team._id}`}>
                    <div className="card card-background p-3">
                      <h5>{team.name}</h5>
